...
class EcmwsModel():
    """ECMWS schedules ready DAG tasks directly with CWS, BLDP, TS3, CCRA and RAPPO."""

    def __init__(self, env, sort_name, iftraining=True):
        self.sort_name = sort_name
        self.env = env
        self.algorithm = ECMWS(is_training=iftraining)

    def learn(self, episode):
        best_reward = float("-inf")
        for i in tqdm(range(episode)):
            logging.info("ECMWS Episode: {}".format(i))
            state = self.env.reset()
            done = False
            total_reward = 0
            assign_num = 0
            ...
            avg_reward = total_reward / assign_num if assign_num != 0 else 0
            logging.info(f"assign num: {assign_num}, avg_reward of ECMWS episode {i}: {avg_reward}")
