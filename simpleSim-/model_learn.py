...
class GasMarlModel():
    """GAS-MARL trains PPO job selection/delay actions and uses heuristic Green-Backfilling."""

    def __init__(self, env, sort_name, iftraining=True):
        self.sort_name = sort_name
        self.env = env
        self.algorithm = GASMARL(is_training=iftraining)

    def learn(self, episode):
        best_reward = float("-inf")
        for i in tqdm(range(episode)):
            logging.info("GAS-MARL Episode: {}".format(i))
            state = self.env.reset()
            done = False
            total_reward = 0.0
            assign_num = 0
            ...
            while not done:
                tasks_to_schedule = self.env.get_tasks_to_execute() + self.env.get_tasks_to_schedule()
                if len(tasks_to_schedule) > 0:
                    tasks_to_schedule = sort_tasks(self.sort_name, tasks_to_schedule)
            ...
            terminal_reward = self.algorithm.finish_episode(self.env)
            total_reward += terminal_reward
            avg_reward = total_reward / assign_num if assign_num != 0 else terminal_reward
            logging.info(
                "assign num: %s, avg_reward of GAS-MARL episode %s: %s, renewable_util: %s, avg_bsd: %s",
                assign_num,
                i,
                avg_reward,
                self.algorithm.last_metrics["renewable_util"],
                self.algorithm.last_metrics["avg_bsd"],
            )
