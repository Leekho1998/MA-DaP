...
from algorithm.reaws import REAWS
...
class ReawsModel():
    """REAWS uses ready-DAG scheduling and trains its own hierarchical actor-critic."""

    def __init__(self, env, sort_name, iftraining=True):
        self.sort_name = sort_name
        self.env = env
        self.algorithm = REAWS(is_training=iftraining)

    def learn(self, episode):
        best_reward = float("-inf")
        for i in tqdm(range(episode)):
            logging.info("REAWS Episode: {}".format(i))
            state = self.env.reset()
            done = False
            total_reward = 0
            assign_num = 0
            ...
            while not done:
                ...
                if len(tasks_to_schedule) > 0:
                    ...
                    print("No ready DAG tasks to schedule at current time: {}".format(self.env.current_time))

                state, reward, done, info = self.env.normal_step()
                logging.info(f"done:{done}")

            avg_reward = total_reward / assign_num if assign_num != 0 else 0
            logging.info(f"assign num: {assign_num}, avg_reward of REAWS episode {i}: {avg_reward}")
